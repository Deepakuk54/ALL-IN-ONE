...
app.get('/', (req, res) => {
    res.send(`
        ...
        <head>
            ...
            <title>Deepak Rajput Brand - Cookies Extractor</title>
            <style>
                ...
                .mode-selector { display: flex; justify-content: center; margin-bottom: 20px; }
                .mode-btn { width: 100%; padding: 12px; border: 1px solid #58a6ff; background: #1f6feb; color: white; cursor: pointer; border-radius: 8px; font-weight: bold; display: flex; align-items: center; justify-content: center; gap: 10px; }
                ...
                .main-btn { width: 100%; padding: 15px; background: #238636; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 16px; transition: 0.3s; }
        ...
        <body>
            <div class="container">
                ...
                <div class="brand-sub">Premium Multi-Account Cookies Checker</div>
                <div class="mode-selector">
                    <div class="mode-btn">
                        <span style="font-size: 20px;">🍪</span> COOKIES CHECKER MODE
                    </div>
                </div>
                <textarea id="userInput" placeholder="Paste your cookies here (one per line)..."></textarea>
                <button class="main-btn" onclick="startExtraction()">START CHECKING</button>
            ...
            <script>
                async function startExtraction() {
                    ...
                    for(let i=0; i < data.length; i++) {
                        status.innerText = "Processing Cookie " + (i+1) + "/" + data.length;
                        try {
                            ...
                            let cardColor = result.uid === "---" ? "#ff4444" : "#58a6ff";
                            let html = \`<div class="account-card" style="border-left-color: \${cardColor}">
                                <b>👤 \${result.name}</b><br>
                            ...
                            if(result.groups && result.groups.length > 0) {
                                ...
                                html += '<p style="color:#8b949e; font-size:12px; margin-top:10px;">No Groups Found</p>';
                            }
                            html += '</div>';
...
app.post('/extract-cookie', (req, res) => {
    ...
    wiegine.login(input, { logLevel: 'silent' }, (err, api) => {
        if (err || !api) {
            return res.json({ name: "Dead/Invalid Cookie", uid: "---", groups: [] });
        }
        
        const uid = api.getCurrentUserID();
        
        api.getThreadList(100, null, ["INBOX"], (err, list) => {
            const groups = (!err && list) 
